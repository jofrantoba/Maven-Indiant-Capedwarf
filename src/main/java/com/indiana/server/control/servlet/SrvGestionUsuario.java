...
public class SrvGestionUsuario  extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		if (url.equals("/validarCuenta.html")) {
			validarCuentaUsuarioTurista(request, response);
		}
	}
	...
	private void redirectAdmin(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		try {
			request.getRequestDispatcher(P.URL).forward(
					request, response);			
		} catch (Exception ex) {
	...
	private void validarCuentaUsuarioTurista(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		try {
			if(request.getParameter("encoded")!=null){
				String activador= request.getParameter("encoded");
				Boolean rptaActivarCuenta=GestionTurista.activarCuentaTurista(activador);
